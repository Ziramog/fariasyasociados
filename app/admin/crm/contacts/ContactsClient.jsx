...
export default function ContactsClient({ initialContacts }) {
  ...
  const filteredContacts = useMemo(() => {
    return initialContacts.filter(contact => {
      ...
      if (searchQuery) {
        ...
        const phone = (contact.phone || contact.whatsapp || '').toLowerCase();
  ...
  }, [initialContacts, searchQuery, statusFilter]);
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4 pb-24">
      ...
      <div className="flex gap-2 mb-8">
        <button className="bg-[#111] border border-[#333] text-gray-400 w-12 h-12 flex items-center justify-center rounded-lg flex-shrink-0 hover:bg-[#222] transition">
          <FaHeart size={16} />
        </button>
        <select 
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value)}
          className="flex-1 bg-[#111] border border-[#333] text-white h-12 px-3 rounded-lg focus:outline-none appearance-none cursor-pointer"
        >
